"""실습 ②: Qwen3-0.6B 아티팩트 빌드 — 독립 실행본

이 파일은 _kit/scripts/build_artifact.py 를 건드리지 않고, Qwen3 전용으로
모델 ID를 박아 넣은 자립 스크립트입니다. (원본은 Qwen2.5 레퍼런스로 동결)

이 실습의 교훈: 모든 모델을 고쳐야 하는 건 아니다.
------------------------------------------------
Qwen2.5-0.5B(실습 ①)는 프리셋에 append_buckets 가 빠져 직접 채워야 했지만,
Qwen3 는 프리셋에 **이미 들어 있다.** 그래서 여기엔 --fix-append 도, 직접 만든
BucketConfig 도 없다. dry-run 으로 "append_buckets 가 0이 아님"을 확인하고 그냥 빌드한다.

사용법
------
  bash ../_kit/scripts/check_resources.sh                # 0) 자원 먼저
  python build_qwen3.py --dry-run                        # 1) 버킷 해석만 확인
  python build_qwen3.py -o ./qwen3-0.6b-artifact         # 2) 실제 빌드
  python ../_kit/scripts/verify_artifact.py --artifact ./qwen3-0.6b-artifact | tee verify.log
"""
...
MODEL_ID = "Qwen/Qwen3-0.6B"  # 작아서 실습에 적당, pod 캐시에 있음
...
def main() -> None:
    ...
    ap.add_argument("-o", "--output", type=Path, default=Path("./qwen3-0.6b-artifact"))
    ...
    # Qwen3 는 프리셋이 완비돼 있으므로 BucketConfig 를 넘기지 않는다(자동 프리셋 적용).
    ...
    print(f"  → 총 파이프라인 약 {total}개  (Qwen2.5의 14개보다 많다 — 그만큼 오래 걸림)")

    if args.dry_run:
        print("\n[dry-run] append_buckets 가 0이 아니면 손댈 필요 없음. 컴파일하지 않음.")
        return
